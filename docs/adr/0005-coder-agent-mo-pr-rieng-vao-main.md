...
- **Cần một điểm review duy nhất, nhất quán** cho mọi tool: PR riêng → Claude Code (hoặc user) review → squash merge vào `main`.
...
2. Mọi PR của Coder agent đều qua review của Claude Code hoặc user trước khi merge — giữ nguyên nguyên tắc "không agent nào tự review code của chính nó".
...
- Quy trình review nhất quán cho mọi tool — Claude Code luôn là điểm chốt cuối trước khi code của agent vào `main`.
