...
>
> 🟦 L07 **CỐ TÌNH giữ ở "Đang học", KHÔNG chuyển ✅ như L05/L06** dù reference implementation NES-8 đã merge (PR #87, `5791085`, CI xanh: `pnpm test` 9 suites/34 tests PASS — lint/prettier/build đều PASS) và lesson note đã closeout dưới ngoại lệ execution-substitute (hands-on NES-84 + quiz NES-88 đều do Hermes/Codex/Claude Code thực thi thay, không phải Hien Duong tự làm — xem disclaimer trong `docs/lessons/07-prisma-postgresql/README.md`). **Lý do khác về bản chất với L05/L06:** hai lesson đó không phụ thuộc runtime DB nên `pnpm test:e2e` đã chạy thật và PASS trong CI của chúng; L07 thì mục tiêu cốt lõi của cả Phase 2 chính là "dữ liệu sống trong Postgres thật qua Prisma" (xem dòng Mục tiêu phía trên) — và chính phần đó **chưa từng được xác minh**: migration `prisma/migrations/20260826162300_init` chưa từng apply lên Postgres thật, `pnpm test:e2e` (đòi Postgres sống) chưa từng chạy được (Docker waived lúc PR #87 thực thi, và `.github/workflows/ci.yml` vẫn comment out service `postgres` + step E2E — gap có từ trước PR này). "Waive Docker + waive learner gate" cùng lúc là mức ngoại lệ **lớn hơn** tiền lệ L05/L06 (chỉ waive learner gate) — không tự động áp dụng cùng một closeout pattern chỉ vì code/CI xanh. L07 chuyển ✅ khi có 1 trong 2: (a) chạy thật `pnpm test:e2e` với Postgres sống (local hoặc CI bật `services.postgres`) và cập nhật lại bằng chứng, hoặc (b) user tường minh duyệt thêm một ngoại lệ riêng cho việc waive runtime DB (khác với ngoại lệ waive learner gate đã có).
>
> 🟦 L08 **giữ ở "Đang học", KHÔNG chuyển ✅.** Lý thuyết (NES-90) đã merge (PR #92, `3186b24`). Khác với callout trước đó của note này: `prisma/schema.prisma`, migration (`prisma/migrations/20260827100000_add_user_project_task_comment_relations`), `prisma/seed.ts`, và DTO/service (`src/tasks/dto/*`, `src/tasks/tasks.service.ts`) **đã được áp dụng** qua PR #91 (Fixes NES-9, merged `2d59255`, CI xanh: `pnpm verify` PASS) — đây là **execution substitute do user duyệt cho đợt này (2026-08-27)**, không phải bằng chứng Hien Duong tự tay làm NES-91. PR #91 tự công bố rõ trong mô tả: `prisma migrate dev`/seed/e2e với PostgreSQL sống là **SKIPPED/UNVERIFIED** (Docker waived). Post-merge review (NES-121, PR sửa lỗi tiếp theo) đã đối chiếu và sửa 3 lỗ hổng bị bỏ sót ở review PR #91 (status/completed không đồng bộ, boolean lọt qua validation projectId/assigneeId, response Task lộ field ngoài spec `{id,title,completed}`) — không đổi lại quyết định "execution substitute" đã có, chỉ sửa chất lượng code của chính phần đã substitute. Post-merge review tiếp theo của PR #93 (NES-122, 2026-08-28) sửa 6 gap còn lại: relation-ID DTO transform từ chối cả input dạng mảng, lệnh readiness Postgres trong docs khớp đúng service `postgres` của `docker-compose.yml`, hướng dẫn seed tường minh 2 lần + sửa mô tả race-condition seed, và ví dụ schema minh hoạ có thêm `completed`. **EN mirror/GitLab cho L08 (PR #91/#92/#93 + sửa NES-122) vẫn chưa làm — cố tình để lại làm follow-up riêng**, chưa có task/PR mirror kế tiếp. L08 chuyển ✅ chỉ sau khi: (a) NES-91 hands-on chạy thật với PostgreSQL sống (migrate/seed/verify bằng `prisma studio` hoặc tương đương) **hoặc** user duyệt thêm ngoại lệ waive-runtime-DB riêng (như đang chờ ở L07), (b) NES-93 review/quiz được Hien Duong tự trả lời thật, và (c) EN mirror/GitLab hoàn tất theo Bilingual Policy.
