...
>
> 🟦 L08 **giữ ở "Đang học", KHÔNG chuyển ✅.** Lý thuyết (NES-90) đã merge (PR #92, `3186b24`). Khác với callout trước đó của note này: `prisma/schema.prisma`, migration (`prisma/migrations/20260827100000_add_user_project_task_comment_relations`), `prisma/seed.ts`, và DTO/service (`src/tasks/dto/*`, `src/tasks/tasks.service.ts`) **đã được áp dụng** qua PR #91 (Fixes NES-9, merged `2d59255`, CI xanh: `pnpm verify` PASS) — đây là **execution substitute do user duyệt cho đợt này (2026-08-27)**, không phải bằng chứng Hien Duong tự tay làm NES-91. PR #91 tự công bố rõ trong mô tả: `prisma migrate dev`/seed/e2e với PostgreSQL sống là **SKIPPED/UNVERIFIED** (Docker waived). Post-merge review (NES-121, PR sửa lỗi tiếp theo) đã đối chiếu và sửa 3 lỗ hổng bị bỏ sót ở review PR #91 (status/completed không đồng bộ, boolean lọt qua validation projectId/assigneeId, response Task lộ field ngoài spec `{id,title,completed}`) — không đổi lại quyết định "execution substitute" đã có, chỉ sửa chất lượng code của chính phần đã substitute. L08 chuyển ✅ chỉ sau khi: (a) NES-91 hands-on chạy thật với PostgreSQL sống (migrate/seed/verify bằng `prisma studio` hoặc tương đương) **hoặc** user duyệt thêm ngoại lệ waive-runtime-DB riêng (như đang chờ ở L07), và (b) NES-93 review/quiz được Hien Duong tự trả lời thật.
