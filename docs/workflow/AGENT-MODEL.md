...
**Exception when Claude is the author:** if a PR (docs/ADR/infrastructure) was created by Claude itself, Claude **does not** review that PR — route it to Hermes manual verify + direct user lead review instead (details: `docs/workflow/REVIEW-MODEL.md`).
...
Used only for the `mr/*` collector branch (max 2 times/day), dispatched interactively via herdr — **not** automatic on every PR, **not** used for small PRs. Details: `docs/workflow/REVIEW-MODEL.md`.
...
> `.github/CODEOWNERS` is also part of `.github/**` → only **Claude Code** creates/edits it, committed with the trailer `Co-authored-by: Claude <noreply@anthropic.com>`. If the Coder agent (codex) produces a change under `.github/**`, it must be recreated through Claude Code before merge — it is not accepted as-is.
