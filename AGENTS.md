...
* Validate data at the input boundary (DTO + `ValidationPipe`) before passing it to the service.- Do not log secrets, tokens, passwords. Do not return `password`/`refreshToken` in responses.
