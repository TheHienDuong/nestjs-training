...
- **Linear MCP is not connected:** notify the user to run `! claude mcp add --transport http linear https://mcp.linear.app/mcp` then `/mcp`. Continue with the remaining steps, use a branch in the format `lesson/XX-lesson-name`, and note that the Linear linking step is still pending.
