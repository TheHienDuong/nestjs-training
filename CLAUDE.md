...
The repo is operated **like a real project** (Linear + GitLab + GitHub integration/review + CI + Slack + Notion) so the user also learns how a backend team works. The final course product is a **Task Management API** (User / Project / Task / Comment).
...
The repo has 2 versions: `main` = Vietnamese, `example/nestjs-training` = English (mirror). When changing any docs/config, update both versions with equivalent content. Code (`src/`, `test/`) is identical across the 2 versions — only docs/config differ by language. GitLab is the daily canonical repository and only accepts the English version from `example/nestjs-training`, author = `hienduong-agility`, no trailer. Details: [docs/bilingual-policy.md](docs/bilingual-policy.md).
...
- Branches must use the **exact** name generated by Linear (`hien/nes-XX-...`). The `nes-XX` string is what Linear uses to automatically transition the issue's status.
- PR description **must** include `Fixes NES-XX`.
- Merge with **Squash and merge**: 1 lesson = 1 commit on `main`.
- **Code owner mandatory approval before merge (2026-08-20):** `@hienduong-agilityio` (`.gitlab/CODEOWNERS` for GitLab MRs; `.github/CODEOWNERS` for GitHub integration PRs) — an additional gate that does not change merge rights (still only the user).
- Remotes: `origin` = GitLab (the daily canonical MR/CI repository), `github` = GitHub (integration and automated review only). Push daily work to `origin`; use `github` only for the integration/review mirror workflow.
