...
- **Mandatory code-owner approval before merge (2026-08-20):** `@hienduong-agilityio` (`.github/CODEOWNERS`) — an additional GitHub gate, does not change merge rights (still only the user). `.github/CODEOWNERS` is part of `.github/**` → only Claude Code creates/edits it, committed with the trailer `Co-authored-by: Claude <noreply@anthropic.com>`.
