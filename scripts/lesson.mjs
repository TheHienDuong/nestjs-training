...
// Empty tree is the Git marker with no files; the first lesson must diff from it.
...
function printList() {
  ...
  if (slugs.size === 0 && tags.size === 0) {
    console.log('  (no lessons yet — run `node scripts/lesson.mjs --tag <NN>` when the lesson is complete)');
    return;
  }
  const rows = [];
  for (const [nn, slug] of [...slugs.entries()].sort()) {
    rows.push([nn, slug, tags.has(`lesson/${nn}`) ? '✅ tagged' : '⬜ not tagged']);
  }
...
function printFileMap(nn, slugs) {
  ...
  if (!tags) {
    ...
    console.error(`   Mark this lesson complete: node scripts/lesson.mjs --tag ${nn}`);
    process.exit(1);
  }
...
function printFile(nn, file, slugs) {
  ...
  try {
    ...
    console.error(`   See the file list for lesson ${nn}: node scripts/lesson.mjs ${nn}`);
    process.exit(1);
  }
...
function createTag(nn) {
  ...
  if (!/^\d{1,2}$/.test(nn)) {
    console.error(`❌ Invalid lesson number: "${nn}" (use 1–2 digits, for example 0 or 01).`);
    process.exit(1);
  }
  const branch = git(['branch', '--show-current']);
  if (branch !== 'main') {
    console.error(`❌ Tags may only be created on main (currently on branch "${branch}").`);
    console.error(`   To create a temporary tag on another branch: git tag lesson/${nn} <commit>`);
    process.exit(1);
  }
  const tag = `lesson/${nn}`;
  if (git(['tag', '-l', tag])) {
    console.error(`❌ Tag ${tag} already exists. Delete it manually to recreate it: git tag -d ${tag}`);
    process.exit(1);
  }
  // A tag points directly to HEAD, so all uncommitted changes must be handled first.
  const dirty = git(['status', '--porcelain', '--untracked-files=all']);
  if (dirty) {
    console.error('❌ Working tree has uncommitted changes; commit or stash them before creating a tag.');
    process.exit(1);
  }
  git(['tag', tag]);
...
try {
  if (arg1 === undefined) {
    ...
    if (!arg2 || !process.argv[4]) {
      console.error('❌ Two lessons are required: node scripts/lesson.mjs --diff <A> <B>');
      process.exit(1);
    }
    printDiff(pad(arg2), pad(process.argv[4]), slugs);
  } else if (arg1 === '--tag') {
    if (!arg2) {
      console.error('❌ A lesson number is required: node scripts/lesson.mjs --tag <NN>');
      process.exit(1);
    }
    createTag(pad(arg2));
  } else if (arg1 === '--note') {
    if (!arg2) {
      console.error('❌ A lesson number is required: node scripts/lesson.mjs --note <NN>');
      process.exit(1);
    }
